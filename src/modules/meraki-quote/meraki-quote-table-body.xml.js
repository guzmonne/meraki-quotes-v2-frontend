...
const hardwareBegins = ({ Devices = [] }, offset) => (
  offset /*Under table*/
  - 2 /*Two Sub headings*/
  - 4 /*Service, Administration, Licenses, and Financing*/
  - Devices.filter(isHardware).length /*Hardware Lenght*/
  - Devices.filter(isLicense).length /*Software Length*/
);

const hardwareEnds = (quote, offset) => (
  hardwareBegins(quote, offset)
  + (quote.Devices || []).filter(isHardware).length /*Hardware Lenght*/
  - 1
);

const licenseBegins = ({ Devices = [] }, offset) => (
  offset /*Under Table*/
  - 1 /*One Sub headings*/
  - 4 /*Service, Administration, Licenses, and Financing*/
  - Devices.filter(isLicense).length /*Software Length*/
...
const licenseEnds = (quote, offset) => (
  licenseBegins(quote, offset)
  + (quote.Devices || []).filter(isLicense).length
  - 1
);
...
<Row>
	...
	<Cell ss:StyleID="s295">
		<Data ss:Type="Number">${calculateServicePrice(quote)}</Data>
	</Cell>
</Row>
`
...
<Row>
	...
	<Cell ss:StyleID="s96"><Data ss:Type="String">-</Data></Cell>
	<Cell ss:StyleID="s295">
		<Data ss:Type="Number">${calculateAdministrationCost(quote)}</Data>
	</Cell>
</Row>
`
